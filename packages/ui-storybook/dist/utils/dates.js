...
export const reverseDate = (value, split = '/', join = '-') => {
    return value === null || value === void 0 ? void 0 : value.split(split).reverse().join(join);
};
const checkLeapYear = (year) => {
    const FEBRUARY_INDEX_MONTH = 1;
    return new Date(year, FEBRUARY_INDEX_MONTH, LEAP_YEAR_DAY).getMonth() === 1;
};
export const validateDate = (value, reverse) => {
    if (!value)
        return false;
    ...
    if (!isLeapYear && month === LEAP_YEAR_MONTH && day >= LEAP_YEAR_DAY)
        return false;
    if (isLeapYear && month === LEAP_YEAR_MONTH && day > LEAP_YEAR_DAY)
        return false;
    if (MONTHS_30_DAYS.indexOf(month) !== -1 && day === MAX_MONTHS_DAYS)
        return false;
    return true;
};
export const validateYearRange = (value, minYear, maxYear) => {
