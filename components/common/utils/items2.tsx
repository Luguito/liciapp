...
export const TestContainer2: FC<testContainerProps> = ({ fn, edit }) => {
    ...
    return (
        <div>
            <TreeView defaultCollapseIcon={<ArrowRightIcon />} defaultExpandIcon={<ArrowDropDownIcon />} sx={{ flexGrow: 1, overflowY: 'auto' }}>
                <TestItem2 items={name} fn={setIndex} setValue={changeInput}></TestItem2>
            </TreeView>
...
export const Item2 = ({ item, fn, id, setValue }) => {
    const dummyOptions = [
        'EXCAVACION Y RETIRO DEL MATERIAL',
        'RELLENO PIEDRA RAJON',
        'RELLENO CON TRITURADO CALIZO TM 3 PULGADAS',
...
export const TestItem2 = ({ items, fn, setValue }) => {
        ...
        <>
            {
                item.child && item.child.length > 0 ? (
                    <>
                        <Item2 item={item} fn={fn} id={item.id} setValue={setValue}></Item2>
                        <TreeItem nodeId={item.id} label={
                            <TestItem2 items={item.child} fn={fn} setValue={setValue}></TestItem2>
                        }>
                        </TreeItem>
                    </>
                ) : (
