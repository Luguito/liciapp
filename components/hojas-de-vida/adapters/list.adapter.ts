...
export const getProfilesByLici: any = async (data: any) => {
    try {
        const { TEAMWORK } = getConfig().publicRuntimeConfig;
        const token = getToken()

        const response = await httpGet(`http://ec2-18-233-75-58.compute-1.amazonaws.com:9050/teamwork/api/project/v1/get-profiles`, token);
        console.log(response)
        return response;
    } catch (e) {
        console.error('err', e);
